...
class TestDistributionTemplateCompleteness:
    ...
    # Map agent name to their command directory structure and file extension
    # Different agents use different paths and formats for storing commands/prompts
    AGENT_CONFIG = {
        'claude': {'path': '.claude/commands', 'ext': '.md'},
        'codex': {'path': '.codex/prompts', 'ext': '.md'},  # codex uses 'prompts' not 'commands'
        'gemini': {'path': '.gemini/commands', 'ext': '.toml'},  # gemini uses TOML format
    }
    ...
    def test_init_generates_all_agent_commands_from_installed_package(self, spec_kitty_repo_root, requires_v011):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for agent, config in self.AGENT_CONFIG.items():
                ...
                # Verify commands were created (path and extension vary by agent)
                commands_dir = project_dir / config['path']
                ...
                # Check for required command templates (extension varies by agent)
                for template_name in self.REQUIRED_TEMPLATES:
                    command_file = commands_dir / f'{template_name}{config["ext"]}'
    ...
    def test_init_generated_implement_follows_workspace_per_wp(self, spec_kitty_repo_root, requires_v011):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Read generated spec-kitty.implement.md
            implement_file = project_dir / '.claude' / 'commands' / 'spec-kitty.implement.md'

            if not implement_file.exists():
                pytest.fail("spec-kitty.implement.md not generated by init")
    ...
    def test_init_generated_review_has_dependency_warnings(self, spec_kitty_repo_root, requires_v011):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Read generated spec-kitty.review.md
            review_file = project_dir / '.claude' / 'commands' / 'spec-kitty.review.md'

            if not review_file.exists():
                pytest.fail("spec-kitty.review.md not generated by init")
    ...
    def test_init_generated_plan_describes_main_repo_workflow(self, spec_kitty_repo_root, requires_v011):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            plan_file = project_dir / '.claude' / 'commands' / 'spec-kitty.plan.md'

            if not plan_file.exists():
                pytest.fail("spec-kitty.plan.md not generated by init")
